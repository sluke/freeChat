...
**您的全功能、便携式终端 AI 聊天利器 - v2.4.0 全屏 TUI 界面版**

`FreeChat` 是一个功能强大、部署简单的单文件 AI 聊天命令行工具，专为在云端 VPS 上使用而设计。通过 SSH 连接后，它为您提供一个全屏 TUI（终端用户界面），包含分屏布局、可滚动的聊天历史和可折叠的侧边栏。

---

## ✨ 核心特性

*   🖥️ **全屏 TUI 界面**: 真正的终端用户界面，分屏布局 — 可滚动聊天区域、常驻顶栏（模型、提示词、费用）、可折叠侧边栏和独立输入区域。告别传统的 REPL 模式。
...
*   🚀 **流式聊天实时更新**: AI 回复直接流式显示在聊天区域，实时刷新。
*   📊 **性能与计费可视化**: 顶栏实时显示当前模型、活跃提示词、会话费用和会话名称。
*   💡 **智能命令行体验**: 输入区域支持命令自动补全（Tab）、历史记录导航（↑↓）和自动建议。
*   ⌨️ **TUI 快捷键**: `F1` 查看帮助，`F2` 切换侧边栏，`Ctrl+Enter` 发送，`Ctrl+D` 退出。
*   💾 **会话管理与导出**: 支持新建、保存和加载聊天会话，并可轻松将会话记录导出为 Markdown, JSON 或 HTML 格式。
...
```bash
# 存储新记忆（支持分类和标签）
> /memory remember "用户喜欢 Python 胜过 JavaScript" --cat preference --tags python,programming
✓ 已记录 (mem_abc123)
  Category: preference
  Tags: python, programming

# 检测重复记忆
> /memory remember "我喜欢 Python 编程"
⚠ Similar memories found (1):
  mem_abc123: 用户喜欢 Python 胜过 JavaScript
Consider updating existing memory with /memory edit
...
# 高级搜索
> /memory search "Python" --cat preference --min-score 0.5
[显示过滤后的记忆]
...
> /memory top 5
#    ID       类别       内容                           分数
1    mem_abc  偏好       用户喜欢深色模式界面            0.92
...

# 查看最近访问的记忆
> /memory recent 5
#    ID       类别       内容                           访问时间
1    mem_abc  偏好       用户喜欢深色模式界面            2024-01-15 14:30
...
...
# 查找相关记忆
> /memory related mem_abc123
Memories related to mem_abc123:
ID       类别       内容                           分数
mem_def  ui         用户喜欢大字体显示              0.78
...
# 编辑记忆
> /memory edit mem_abc123 "用户喜欢深色模式界面和自动补全功能"
✓ Memory updated

# 设置标签
> /memory tag mem_abc123 ui preference dark-mode
✓ Tags updated: ui, preference, dark-mode

# 修改分类
> /memory category mem_abc123 user_profile
✓ Category updated to 'user_profile'

# 设置重要性
> /memory priority mem_abc123 9
✓ Importance set to 9/10
...
# 合并分支记忆
> /memory merge feature/new-ui main
✓ 合并了 3 条记忆从 'feature/new-ui' 到 'main'

# 导出记忆
> /memory export my_memories.json
✓ Exported 42 memories to my_memories.json

# 导入记忆
> /memory import my_memories.json
✓ Imported 42 memories from my_memories.json
...
│ > _                                                          │
├─────────────────────────────────────────────────────────────┤
│ F1=帮助  F2=侧边栏  Ctrl+Enter=发送  Ctrl+D=退出            │
└─────────────────────────────────────────────────────────────┘
```

*   **聊天**: 在输入区域输入消息，按 `Ctrl+Enter` 发送。AI 回复会流式显示在聊天区域中。
*   **命令**: 所有特殊功能通过斜杠命令 `/` 实现。输入 `/` 后按 `Tab` 可自动补全。
*   **侧边栏**: 按 `F2` 切换侧边栏，显示可用命令和会话状态。
*   **帮助**: 按 `F1` 或输入 `/help` 查看命令列表。
...
FreeChat 支持导出带有渲染 Markdown 内容的会话。使用 `/export md-rendered` 命令，将生成包含格式化 Markdown 内容的 HTML 文件。
...
| `/memory` | `remember <text> [--cat <c>] [--tags <t1,t2>]` | 存储新记忆，支持分类和标签，自动检测重复。 |
| | `recall <query>` | 使用全文搜索查找记忆。 |
| | `search <query> [opts]` | 高级搜索，支持 `--cat`, `--tags`, `--min-score`, `--min-importance`。 |
| | `list [category]` | 列出所有记忆，可选按分类筛选。 |
| | `top [n]` | 列出最高价值的 n 条记忆。 |
| | `recent [n]` | 列出最近访问的 n 条记忆。 |
| | `categories` | 列出所有分类及记忆数量。 |
| | `view <id>` | 查看单条记忆的详细信息。 |
| | `edit <id> <content>` | 编辑记忆内容。 |
| | `tag <id> <tags...>` | 设置记忆标签。 |
| | `category <id> <cat>` | 修改记忆分类。 |
| | `priority <id> <1-10>` | 设置记忆重要性（1-10）。 |
| | `related <id>` | 查找与指定记忆相关的记忆。 |
| | `forget <id>` | 删除指定 ID 的记忆。 |
| | `restore <id>` | 恢复已归档的记忆。 |
| | `compress` | 运行基于拍卖算法的压缩，归档低价值记忆。 |
| | `clear --force` | 清空所有记忆（需确认）。 |
| | `stats` | 显示记忆统计信息。 |
| | `branch` | 列出包含记忆的 Git 分支。 |
| | `merge <from> <to>` | 将记忆从一个分支合并到另一个分支。 |
| | `export <file.json>` | 导出记忆到 JSON 文件。 |
| | `import <file.json>` | 从 JSON 文件导入记忆。 |
